...
| 13 | Overflow | **Pick from overflow until it is empty; new inbound always goes to the original rack** | **Supersedes §7.5's replenishment.** No replenishment task; the low threshold is removed; registry keeps two numbers (full, restock) |
...
**On decision 13 — one FIFO edge case, accepted.** Overflow is usually the same delivery
as the stock on the rack, so picking it first is FIFO-correct. The exception: if a
delivery overflows a rack that still holds an *older* batch, picking overflow first
takes the newer units before the older ones on the rack. The older batch waits only
until overflow empties. With 24–36 month shelf life and no expiry tracked (decision 15),
that delay is acceptable. Its real benefit is that overflow slots are freed quickly,
which matters with 57 spare positions and a second brand inside three months.
